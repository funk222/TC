...
| 9 | MAX31865 CS | PT100模块 | CS | ✅ | PT100片选 |
| 10 | 编码器 CLK | 输入 | CLK/A | ✅ | 旋转检测 |
| 11 | 编码器 DT | 输入 | DT/B | ✅ | 旋转检测 |
| 12 | 确认按键 | 输入 | BTN1 | ✅ | 主要操作 |
| 13 | 返回按键 | 输入 | BTN2 | ✅ | 返回/取消 |
| 15 | MAX31865 MOSI | PT100模块 | SDI/MOSI | ✅ | PT100数据输入 |
| 21 | 编码器 SW | 输入 | SW | ✅ | 选择/确认执行 |
...
│ VIN         ├────┤ 3.3V        │
│ GND         ├────┤ GND         │
│ SCK         ├────┤ GPIO 6      │  ← 与MAX6675共用
│ SDO (MISO)  ├────┤ GPIO 4      │  ← 与MAX6675共用
│ SDI (MOSI)  ├────┤ GPIO 15     │
│ CS          ├────┤ GPIO 9      │
└─────────────┘    └─────────────┘
```
...
**注意事项**:
- PT100 建议 3 线接法（与当前固件配置一致）
- 与 MAX6675 共用 SCK/MISO 是当前设计的一部分
- 如 PT100 故障，固件会按容错逻辑处理并在界面显示 `T2 ERROR`
...
### 选项A：使用新配置（推荐）
- ✅ I2C: GPIO 22/23
- ✅ PT100: CS/MOSI/SDO/SCK = GPIO 9/15/4/6
- ✅ RGB LED: GPIO 8 (板载)
- ✅ 功能完整
...
1. 修改 `include/config.h`:
   ...
   #define I2C_SDA 8
   #define I2C_SCL 9
   #define RGB_LED_PIN <unused_gpio>  // 外接LED，避免与PT100引脚冲突
   ```
2. 同时重新规划 PT100 和 RGB 的引脚，避免与现有 SPI 接线冲突
